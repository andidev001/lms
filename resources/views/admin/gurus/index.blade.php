...
@section('subtitle', 'Kelola informasi dan akun guru pengajar')
...
<div class="card border-0 shadow-sm rounded-4">
    <div class="card-header bg-white border-0 p-4 d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
        ...
        <div class="d-flex flex-wrap gap-2">
            ...
            <form action="{{ route('gurus.generate-all') }}" method="POST" class="d-inline">
                @csrf
                <button type="submit" class="btn btn-warning rounded-pill px-4" onclick="return confirm('Sistem akan membuatkan akun untuk semua guru yang belum memiliki akun. Lanjutkan?')">
                    <i class="bi bi-magic me-1"></i> Generate Semua Akun
                </button>
            </form>
            <button type="button" class="btn btn-success rounded-pill px-4" data-bs-toggle="modal" data-bs-target="#importGuruModal">
                <i class="bi bi-file-earmark-excel me-1"></i> Impor Excel
            </button>
            <button type="button" class="btn btn-primary rounded-pill px-4" data-bs-toggle="modal" data-bs-target="#addGuruModal">
...
$(document).ready(function() {
    var table = $('#gurus-table').DataTable({
        processing: true,
        serverSide: true,
    ...
    $('#gurus-table').on('change', '.item-checkbox', function() {
    ...
    $('#btn-bulk-delete').on('click', function() {
        ...
        if (ids.length > 0) {
            Swal.fire({
                ...
                text: 'Apakah Anda yakin ingin menghapus ' + ids.length + ' data guru terpilih?',
                icon: 'warning',
      ...
      <form method="POST" action="{{ route('gurus.store') }}">
          ...
          <div class="modal-body p-4">
              <div class="mb-3">
                  ...
                  <input type="text" name="nip" class="form-control form-control-lg rounded-3" placeholder="Masukkan NIP" value="{{ old('nip') }}" required>
              </div>
              <div class="mb-3">
              ...
              <div class="mb-3">
                  <label class="form-label fw-semibold text-muted">Nomor Telepon (Opsional)</label>
                  <input type="text" name="telepon" class="form-control form-control-lg rounded-3" placeholder="Contoh: 08123..." value="{{ old('telepon') }}">
              </div>
          </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        var myModal = new bootstrap.Modal(document.getElementById('addGuruModal'), {
            keyboard: false
        });
        myModal.show();
    });
...
<div class="modal fade" id="importGuruModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content border-0 shadow rounded-4">
      <div class="modal-header border-bottom-0 pt-4 pb-0 px-4">
        <h5 class="modal-title fw-bold">Impor Data Guru</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form method="POST" action="{{ route('gurus.import') }}" enctype="multipart/form-data">
          @csrf
          <div class="modal-body p-4">
              <div class="alert alert-info rounded-3">
                  <i class="bi bi-info-circle me-1"></i> Silakan unduh template Excel terlebih dahulu sebelum mengunggah data. 
                  <a href="{{ route('gurus.template') }}" class="fw-bold alert-link">Unduh Template</a>
              </div>
              <div class="mb-3">
