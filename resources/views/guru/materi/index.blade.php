        ...
        @forelse ($materis as $materi)
            <div class="col-12 mb-4">
                <div class="card border-0 shadow-sm rounded-4 overflow-hidden h-100">
                    <div class="row g-0">
                        <div
                            class="col-md-1 bg-primary bg-opacity-10 d-flex align-items-center justify-content-center p-3 border-end">
                        ...
                        <div class="col-md-8 p-4">
                            ...
                            <div class="d-flex flex-wrap gap-2 mb-0">
                                @if($materi->file_pdf)
                                    <a href="javascript:void(0)" data-bs-toggle="modal" data-bs-target="#modalPdf{{ $materi->id }}"
                                        class="badge bg-danger bg-opacity-10 text-danger border border-danger border-opacity-25 rounded-pill px-3 py-2 text-decoration-none shadow-sm"
                                ...
                                <span
                                    ...
                                    <i class="bi bi-question-circle-fill me-1"></i> {{ $materi->pretest_questions()->count() }}
                                    Soal Post-Test
                                </span>
                                @if($materi->tenggat_waktu)
                                    <span
                                        ...
                                        {{ \Carbon\Carbon::parse($materi->tenggat_waktu)->format('d M Y H:i') }}
                                    </span>
                                @endif
                            </div>
                        </div>
                        <div
                            class="col-md-3 bg-light p-4 d-flex flex-column justify-content-center align-items-end border-start">
                            <a href="{{ route('guru.mapels.materis.show', [$mapel->id, $materi->id]) }}"
                                class="btn btn-primary w-100 mb-2 rounded-3">
                                <i class="bi bi-gear-fill me-1"></i> Kelola & Soal
                            </a>
                            <a href="{{ route('guru.mapels.materis.edit', [$mapel->id, $materi->id]) }}"
                                class="btn btn-outline-secondary w-100 mb-2 rounded-3">
                                <i class="bi bi-pencil-square me-1"></i> Edit Detail
                            </a>
                            <form action="{{ route('guru.mapels.materis.destroy', [$mapel->id, $materi->id]) }}" method="POST"
                                class="w-100">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-outline-danger w-100 rounded-3"
                                    onclick="return confirm('Yakin ingin menghapus materi ini beserta soal-soalnya? (Data progres siswa terkait juga akan terhapus)')">
            ...
            @if($materi->file_pdf)
                <!-- Modal PDF Preview -->
                <div class="modal fade" id="modalPdf{{ $materi->id }}" tabindex="-1"
                    aria-labelledby="modalPdfLabel{{ $materi->id }}" aria-hidden="true">
                    <div class="modal-dialog modal-xl modal-dialog-centered">
                        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
                            <div class="modal-header bg-dark text-white border-0 py-3 px-4">
                                <h5 class="modal-title fw-bold text-white mb-0 d-flex align-items-center"
                                    id="modalPdfLabel{{ $materi->id }}">
                                    <i class="bi bi-file-earmark-pdf-fill text-danger fs-4 me-2"></i> Preview PDF:
                                    {{ $materi->judul }}
                                </h5>
                            ...
                            <div class="modal-body p-0 bg-secondary bg-opacity-10" style="height: 80vh;">
                                <iframe src="{{ asset('storage/' . $materi->file_pdf) }}" width="100%" height="100%"
                                    style="border: none; min-height: 500px;">
                                    Browser Anda tidak mendukung iframe. <a href="{{ asset('storage/' . $materi->file_pdf) }}"
                                        target="_blank">Download PDF di sini</a>.
                                </iframe>
                            </div>
            ...
            <div class="col-12">
                <div class="card border-0 shadow-sm rounded-4 text-center p-5">
                    <div class="py-5">
                        ...
                        <a href="{{ route('guru.mapels.materis.create', $mapel->id) }}"
